...
def new_component():
    ...
    if new_comp["name"] in list_all_components:
        write_log_warning("{template_warning}: Component ({name})  is already registered".format(template_warning=BAD_NEWENTRY_STR.format(entry="component", username=username), name=new_comp["name"]))
    ...
    new_comp["type"] = int(request.form["comptype"])
